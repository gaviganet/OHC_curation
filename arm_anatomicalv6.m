...
function arm_anatomicalv6(dirname, filename_fits,k_adult_male,count,pathfunctions,pathbegdata,pathsavedata)
...
dadefinition= char(description);
write_attribute_for_group(group_id_2d,dadefinition,ATTRIBUTE);
group_id_2e = H5G.create(fileID, '/anatomical/cochlea/position', 'H5P_DEFAULT', 'H5P_DEFAULT', 'H5P_DEFAULT');
ATTRIBUTE      = 'definition';
...
dadefinition= char(didefinition);
write_attribute_for_group(group_id_2e,dadefinition,ATTRIBUTE);
ATTRIBUTE      = 'imported_from';
...
name_def='position relative to head';
...
ATTRIBUTE='definition';
didefinition= 'cochlea of the test subject used. Legal values are left side of head, right side of head or unknown side'; 
dadefinition= char(didefinition);
specific_string_attribute(DATASETID,ATTRIBUTE,dadefinition)
% %
...
DATASETID=create_and_write_string_dataset(group_id_2d,space,type,name_def,cochlear_region);
% % add attributes
% attribute_general(DATASETID,researcher, dofexp, cellnumber, datasteward, datacurator,funder);% calls a function to add attributes
ATTRIBUTE      = ' alternative definition';
description= 'Subdivision of cochlea possible values are basal, apical or unknown origin of cell used)'; 
dadefinition= char(description);
specific_string_attribute(DATASETID,ATTRIBUTE,dadefinition)
ATTRIBUTE      = ' definition';
